...
import { Shield, LogOut, Briefcase, Users, FolderKanban, CheckSquare, MessageSquare, Bell, Sliders } from 'lucide-react'
...
export default function ManagerDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (parsed.role !== 'Team Manager' && parsed.role !== 'Super Admin' && parsed.role !== 'Admin') {
        setUnauthorized(true)
      } else {
        setSession(parsed)
      }
    } else {
      // If no session, allow fallback or redirect to login
      const cookies = document.cookie
      if (!cookies.includes('webotixs_role_session')) {
        router.push('/admin/login')
      } else {
        setSession({ role: 'Team Manager', name: 'Authorized Manager', email: 'manager@webotixs.com' })
      }
    }
  }, [router])
    ...
    return (
      <div className="min-h-screen bg-[#050816] flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="bg-[#0D1224] border border-red-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl space-y-5">
          ...
          <h1 className="font-display text-xl font-bold text-white mt-2">Team Manager Access Only</h1>
          <p className="text-xs text-[#94A3B8] leading-relaxed">
            You do not have permission to view the Team Manager Dashboard. Please log in with a Team Manager account or return to your assigned portal.
          </p>
          <button
      ...
      <header className="sticky top-0 z-50 bg-[#0D1224]/90 backdrop-blur-xl border-b border-[#273449] px-4 md:px-8 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-600 to-blue-500 flex items-center justify-center text-white shadow-md">
              <Briefcase size={18} />
            </div>
            <div>
              <span className="font-display font-bold text-base text-white tracking-tight">Team Manager OS</span>
              <span className="ml-2.5 px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-400 text-[10px] font-bold uppercase">
                Management Portal
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-2 bg-[#050816] px-3.5 py-1.5 rounded-xl border border-[#273449] text-xs">
            <span className="text-[#94A3B8]">Active Manager:</span>
            <span className="text-white font-semibold">{session?.name || session?.email || 'Authorized Manager'}</span>
          </div>
