...
export default function AdminIndustriesPage() {
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      setIndustries((prev) => [...prev, { ...(editing as Industry), id: crypto.randomUUID(), slug, created_at: now }])
    } else {
      setIndustries((prev) => prev.map((i) => i.id === editing.id ? { ...i, ...editing, slug } : i))
    }
    setSaving(false)
    close()
  }

  const handleDelete = (id: string) => setIndustries((prev) => prev.filter((i) => i.id !== id))
  const toggleStatus = (id: string) => setIndustries((prev) => prev.map((i) => i.id === id ? { ...i, status: i.status === 'published' ? 'draft' : 'published' } : i))
