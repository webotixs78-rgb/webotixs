...
export default function AdminCareersPage() {
  ...
  const handleDeleteJob = (id: string) => setPositions((prev) => prev.filter((p) => p.id !== id))
  const toggleJobStatus = (id: string) =>
    setPositions((prev) => prev.map((p) => (p.id === id ? { ...p, status: p.status === 'published' ? 'closed' : 'published' } : p)))
  ...
  const handleSaveJob = async () => {
    ...
    if (isNewJob) {
      ...
      setPositions((prev) => prev.map((p) => (p.id === editingJob.id ? { ...p, ...editingJob } : p)))
    }
    setSaving(false)
    setEditingJob(null)
  }
