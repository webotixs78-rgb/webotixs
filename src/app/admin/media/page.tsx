...
export default function AdminMediaPage() {
  ...
  useEffect(() => {
    getCMSData<MediaItem[]>('media').then((data) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i]
      const folderTarget = activeFolder === 'All Media' ? 'uploads' : activeFolder
      ...
      await saveCMSItem('media', newItem)
    }
