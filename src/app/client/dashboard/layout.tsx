...
import { Shield, LogOut, Building2, User, Sparkles, Sliders } from 'lucide-react'
...
export default function ClientDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (parsed.role !== 'Client' && parsed.role !== 'Super Admin' && parsed.role !== 'Admin') {
      ...
      if (!cookies.includes('webotixs_role_session')) {
        ...
        setSession({ role: 'Client', name: 'Valued Client', email: 'client@company.com' })
      }
    }
  }, [router])
    ...
    return (
      <div className="min-h-screen bg-[#050816] flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="bg-[#0D1224] border border-red-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl space-y-5">
          ...
          <h1 className="font-display text-xl font-bold text-white mt-2">Client VIP Access Only</h1>
          <p className="text-xs text-[#94A3B8] leading-relaxed">
            You do not have permission to view the Client VIP Dashboard. Please log in with your assigned Client credentials.
          </p>
          <button
    ...
    <div className="min-h-screen bg-[#050816] text-[#E2E8F0] font-sans flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-[#0D1224]/90 backdrop-blur-xl border-b border-[#273449] px-4 md:px-8 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white shadow-md font-display font-bold">
            <Sparkles size={18} />
          </div>
          <div>
            <span className="font-display font-bold text-base text-white tracking-tight">Client VIP Portal</span>
            <span className="ml-2.5 px-2 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[10px] font-bold uppercase">
              Client Board
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-2 bg-[#050816] px-3.5 py-1.5 rounded-xl border border-[#273449] text-xs">
            ...
            <span className="text-[#94A3B8]">Company Account:</span>
            <span className="text-white font-semibold">{session?.name || session?.email || 'Authorized Client'}</span>
          </div>
